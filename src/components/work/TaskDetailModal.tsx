...
import type { Task, Project } from "@/store/workStore";
...
export default function TaskDetailModal({ task, projects, open, onClose, onUpdate, onDelete }: Props) {
  ...
  return (
    <Modal open={open} onClose={() => { setEditing(false); onClose(); }} title={editing ? "Edit Task" : "Task Detail"} width="md">
      {editing ? (
        ...
        <div className="space-y-5">
          ...
          <div className="grid grid-cols-2 gap-4">
            <InfoRow label="Status" value={task.status.replace("_", " ")} capitalize />
            <InfoRow label="Priority" value={<PriorityBadge priority={task.priority} />} />
            {task.project_name && <InfoRow label="Project" value={task.project_name} />}
            {dueDate && <InfoRow label="Due date" value={dueDate} />}
            {task.tags && <InfoRow label="Tags" value={task.tags} />}
          </div>
